...
public:
    ...
    operator bool() const { 
        ...
        if(idouble::recording && CAAD_iBoolVarIsRandom(&val)) {
            std::cout << "ibool -> bool conversion during recording" << std::endl;
            std::raise(SIGINT);
        }
#endif
        return val;
...
inline ibool operator < (const idouble& a, const double& b) {
    return a < idouble(b);
}
inline ibool operator < (const double& a, const idouble& b) {
    return idouble(a) < b;
}
